...
def convertDf(df):
    ...
    for name in df.columns[1:-1]+['""""quality"""""']:
...
def randomForestAlg(data_set):
    ...
    rf_model = RandomForest.trainClassifier(data_set, categoricalFeaturesInfo={}, featureSubsetStrategy="auto", numClasses = 10, numTrees = 21, maxDepth = 30, maxBins = 32, impurity = 'gini')
    
    return rf_model


def rfPredictions(rf_model, test_set):
    predictions = rf_model.predict(test_set.map(lambda x: x.features))
    lap = test_set.map(lambda lp: lp.label).zip(predictions)
    lap_df = lap.toDF()
...
def main():
    ...
    # Parallellize data points
    logMessage("Parallellize Data Points")
    ...
    logMessage("Random Forest Algorithm")
    rf_model = randomForestAlg(train_set)
    print("Trained Model Successfully")

    # Predictions
    logMessage("Making Predictions")
    lap, label_pred_df = rfPredictions(rf_model, test_set)
    ...
    err = lap.filter(lambda lp: lp[0] != lp[1]).count() / float (test_set.count())
